...
bool WiFiManager::wifiConnectDefault(){
  ...
  // AN -
  LCD.setCursor(0,0);
  LCD.print("Connecting to SAVED AP: " + WiFi_SSID(true));
  // An - End
...
uint8_t WiFiManager::waitForConnectResult(uint32_t timeout) {
  if (timeout == 0){
    ...
    // AN -
    LCD.setCursor(0,0);
    LCD.print("connectTimeout not set, ESP waitForConnectResult..."); 
    // AN - End
    #endif
  ...
  DEBUG_WM("Erasing");
    ...
    if(opt){
      ...
      if(SPIFFS.begin()){
        ...
        LCD.print("[ERROR] Could not start SPIFFS");
        #endif
      }
      return ret;
    }
...
boolean WiFiManager::validApPassword(){
  ...
  if (_apPassword != "") {
    if (_apPassword.length() < 8 || _apPassword.length() > 63) {
      ...
      LCD.setCursor(0,0);
      LCD.print("AccessPoint set password is INVALID or <8 chars");
      #endif
      _apPassword = "";
...
void WiFiManager::handleUpdating(){
  ...
  // UPLOAD ABORT
    ...
    LCD.print("[OTA] Update was aborted");
    error = true;
  }
